...
export async function deployChild(
  name: string,
  childCollectionMetadata: String,
): Promise<AriaBody> {
...
export async function deployManagerV2(): Promise<AgeOfChronosManagerV2> {
  console.log(`Deploying AgeOfChronosManager to ${network.name} blockchain...`);
  ...
  console.log(`AgeOfChronosManagerV2 deployed to ${managerAddress}`);
  ...
  if (!isHardhatNetwork()) {
    ...
    await run('verify:verify', {
      ...
      constructorArguments: [],
      contract: 'contracts/manager/AgeOfChronosManagerV2.sol:AgeOfChronosManagerV2',
    });
  }
...
export async function configureManager(
  parent: TimeSquadAria | TimeSquadLuna | TimeSquadRyker | TimeSquadThaddeus,
  childs:
    | AriaBody[]
    | LunaBody[]
    | RykerBody[]
    ...
    | RykerRightHand[]
    | ThaddeusRightHand[],
  manager: AgeOfChronosManager | AgeOfChronosManagerV2,
): Promise<void> {
...
export async function setEquippableAddresses(
  catalog: RMRKCatalogImpl,
  ...
  try {
    const tx = await catalog.setEquippableAddresses(C.SQUAD_BODY_SLOT_PART_ID, [
      equippableChildAddressBody,
    ]);
    await tx.wait();
  } catch (error) {
  ...
  try {
    const tx = await catalog.setEquippableAddresses(C.SQUAD_HEAD_SLOT_PART_ID, [
      equippableChildAddressHead,
    ]);
    await tx.wait();
  } catch (error) {
    console.error('Error setting equippable addresses:', error);
  ...
  try {
    const tx = await catalog.setEquippableAddresses(C.SQUAD_LEFT_HAND_SLOT_PART_ID, [
      equippableChildAddressLeftHand,
    ]);
    await tx.wait();
  } catch (error) {
    console.error('Error setting equippable addresses:', error);
  ...
  try {
    const tx = await catalog.setEquippableAddresses(C.SQUAD_RIGHT_HAND_SLOT_PART_ID, [
      equippableChildAddressRightHand,
    ]);
    await tx.wait();
  } catch (error) {
    console.error('Error setting equippable addresses:', error);
...
export async function mintChildNFTWithNewAssets(
  child: AriaBody,
  recipient: string,
  assetIds: number[],
) {
