...
async function main() {
  ...
  const CalculateBalanceContractAddress = "0x985d4479786c696b3a1b36f7e372184bf6e503ce";
  ...
  // Calcolo del balance
  const totalBalance = await calculateBalance.calculateBalance(directOwnerAddress, collectionParentAddresses, childAddress);
  const balance = await rykerRightHand.balanceOf(directOwnerAddress);
  const balanceDirectOwnerOf = await rykerRightHand.ownerOf(directOwnerAddress);
  console.log('Total Balance:', totalBalance.toString());
  console.log('Balance:', balance.toString());
  console.log('balanceDirectOwnerOf:', balanceDirectOwnerOf.toString());
  
  // Utilizzo della funzione tokenOfOwnerByIndex in un ciclo for
  for (let i = totalBalance - 1n; i >= 0; i--) {
      //const tokenId = await rykerRightHand.tokenOfOwnerByIndex(directOwnerAddress, i);
      const tokenId = await rykerRightHand.tokenByIndex(i);
