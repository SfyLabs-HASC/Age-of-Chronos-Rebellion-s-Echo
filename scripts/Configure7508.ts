...
async function main() {
  ...
  const TOKEN_ATTRIBUTES_REPOSITORY_ADDRESS_MOONBEAM = '0x4778B7e8088B258A447990e18AdB5fD14B1bD100';
  ...
  const ariaBody: AriaBody = await ethers.getContractAt(
    'AriaBody',
    contractItemAddresses.AriaBody,
    deployer,
  );
  const ariaHead: AriaHead = await ethers.getContractAt(
    'AriaHead',
    contractItemAddresses.AriaHead,
    deployer,
  );
  ...
  const lunaBody: LunaBody = await ethers.getContractAt(
    'LunaBody',
    contractItemAddresses.LunaBody,
    deployer,
  );
  const lunaHead: LunaHead = await ethers.getContractAt(
    'LunaHead',
    contractItemAddresses.LunaHead,
    deployer,
  );
  ...
  const manager: AgeOfChronosManager = await ethers.getContractAt(
    'AgeOfChronosManager',
    managerAddress,
    deployer,
  );
  const tokenAttributesContract: RMRKTokenAttributesRepository = await ethers.getContractAt(
    'RMRKTokenAttributesRepository',
    TOKEN_ATTRIBUTES_REPOSITORY_ADDRESS_MOONBEAM,
    deployer,
  );
  ...
  if (firstTime) {
    for (const [key, address] of Object.entries(contractParentAddresses)) {
      ...
      const tx = await tokenAttributesContract.registerAccessControl(
        address,
        deployer.address,
        true,
      );
    ...
    for (const [key, address] of Object.entries(contractParentAddresses)) {
      ...
      await tx.wait();
      console.log(
        `manageAccessControl: ${deployer.address} is set as ${accessTypeOwner} for ${key} collection`,
      );
      delay(5000);
    }
    ...
    for (const [key, address] of Object.entries(contractParentAddresses)) {
      ...
      await tx.wait();
      console.log(
        `manageAccessControl: ${vicedirettoreAddress} is set as ${accessTypeCollaborator} for ${key} collection`,
      );
      delay(5000);
    }
    ...
    for (const [key, address] of Object.entries(contractParentAddresses)) {
      ...
      console.log(
        `${key} collection (address: ${address}): ${whois ? 'vicedirettoreAddress is a collaborator' : 'Deployer is not a collaborator'}`,
      );
      if (!whois) {
        //Only the owner of the collection can call this function
        ...
        await tx.wait();
        console.log(
          `manageCollaborators: ${vicedirettoreAddress} is set as ${true} for ${key} collection`,
        );
        delay(5000);
      }
    }
  ...
  for (const [key, address] of Object.entries(contractParentAddresses)) {
    ...
    await tx.wait();
    console.log(
      `manageAccessControl: ${managerAddress} is set as ${accessTypeCollaborator} for ${key} collection`,
    );
    delay(2000);
  }
  ...
  for (const [key, address] of Object.entries(contractParentAddresses)) {
    ...
    console.log(
      `${key} collection (address: ${address}): ${whois ? 'managerAddress is a collaborator' : 'Deployer is not a collaborator'}`,
    );
    if (!whois) {
      //Only the owner of the collection can call this function
      let tx = await tokenAttributesContract.manageCollaborators(address, [managerAddress], [true]);
      await tx.wait();
  ...
  for (const [key, address] of Object.entries(contractParentAddresses)) {
    ...
    console.log(
      `${key} collection TokenID: ${tokenId} (address: ${address}):whichLevel: ${whichLevel}`,
    );
  }

  console.log('provo col videdirettore');

  const tokenAttributesContractAddr1: RMRKTokenAttributesRepository = await ethers.getContractAt(
    'RMRKTokenAttributesRepository',
    TOKEN_ATTRIBUTES_REPOSITORY_ADDRESS_MOONBEAM,
    addr1,
  );
  ...
  for (const [key, address] of Object.entries(contractParentAddresses)) {
    const resultato = await tokenAttributesContractAddr1.setUintAttribute(
      address,
      tokenId,
      attributesKey,
      0,
  ...
  async function manageAccessControl(
    collection: string,
    key: string,
    accessType: number,
  ...
  async function manageCollaborators(
    collection: string,
    collaboratorAddresses: string[],
    collaboratorAddressAccess: boolean[],
  ) {
    await tokenAttributesContract.manageCollaborators(
      collection,
  ...
  async function setStringAttribute(
    collection: string,
    tokenId: number,
    key: string,
    value: string,
  ...
  async function setAddressAttribute(
    collection: string,
    tokenId: number,
    key: string,
    value: string,
  ...
  async function setIntAttributes(
    collections: string[],
    tokenIds: number[],
  ...
  async function getUintAttribute(
    collection: string,
    tokenId: number,
    key: string,
  ): Promise<BigInt> {
  ...
  async function getAddressAttribute(
    collection: string,
    tokenId: number,
    key: string,
  ): Promise<string> {
