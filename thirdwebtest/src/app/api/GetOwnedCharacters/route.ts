...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await getOwnedCharacters(parentContract, walletAddress);
...
async function getOwnedCharacters(parentContract: string, walletAddress: string) {
  ...
  try {
    const response = await axios.get(`${engineBaseUrl}/contract/${chain}/${parentContract}/erc721/get-owned`, {
      headers: {
